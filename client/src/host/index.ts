	...
	return new Proxy(target, {
		get(obj, prop, receiver) {
			...
			if (value instanceof Uri) {
				console.log("🚀 --> Uri:", uriToString(positionDetails.uri), uriToString(value), uriToString(positionDetails.uri) === uriToString(value));
			...
			} else if (value instanceof Uri && uriToString(positionDetails.uri) === uriToString(value)) {
		...
		set(obj, prop, value, receiver) {
			if (value instanceof Position || value instanceof Uri || value instanceof Range || prop in state) {
...
export class TextDocumentController {
	...
	provideDefinition(position: Position, uri: Uri) {
		...
		return commands.executeCommand<Definition | DefinitionLink[]>(
			...
			console.log("🚀 --> file: index.ts:186 --> TextDocumentController --> provideDefinition --> definitions:", definitions);
